...
namespace Mapbox.Unity.Map
{
    ...
    public class AbstractMap : MonoBehaviour, IMap
	{
        ...
        float maxZoomValue = 22f;
        void Update()
        {
            /*Vector3? translatePosition = null;
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                Vector2 touchDeltaPosition = Input.GetTouch(0).deltaPosition;
                translatePosition = new Vector3(-touchDeltaPosition.x * speed, -touchDeltaPosition.y * speed, 0);
            }
            if (Input.GetKey(KeyCode.RightArrow))
            {
                translatePosition = new Vector3(speed, 0, 0);
            }
            ...
            {
                translatePosition = new Vector3(0, -speed, 0);
            }
            ...
            if (translatePosition != null)
            {
                ...
                if (camera != null)
                {
                    camera.transform.Translate((Vector3)translatePosition);
                }
            }
            if (Input.GetAxis("Mouse ScrollWheel") != 0f)
            {
                //mouse was scrolled
                //zoom in/out map
                float zoomVal = Options.locationOptions.zoom + Input.GetAxis("Mouse ScrollWheel");
                UpdateMap(zoomVal);
            }*/
        }
